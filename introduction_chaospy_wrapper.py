...
poly = cp.generate_expansion(polynomial_order, n, rule='three_terms_recurrence', normed=True)
print('Discretized Stieltjes / Three terms reccursion {}'.format(poly))
ax = plt.subplot(132)
ax.set_title('Discretized Stieltjes ')
_=plt.plot(x, poly(x).T)

# TODO: Gram-Schmidt is broken
...
basis = cpw.generate_basis(polynomial_order, joint_distribution)

# 3.1 generate samples
number_of_samples = len(basis['poly']) 
...
gpce_regression = cpw.fit_regression(basis, samples, model_evaluations)
print("Success")
...
gpce_quadrature = cpw.fit_quadrature(basis, nodes, weights, model_evaluations)
print("Success")
...
exp_reg = cpw.E(gpce_regression, joint_distribution)
exp_ps =  cpw.E(gpce_quadrature, joint_distribution)

std_reg = cpw.Std(gpce_regression, joint_distribution)
str_ps = cpw.Std(gpce_quadrature, joint_distribution)

prediction_interval_reg = cp.Perc(gpce_regression['approx'], [5, 95], joint_distribution)
prediction_interval_ps = cp.Perc(gpce_quadrature['approx'], [5, 95], joint_distribution)
...
# example sens
sensFirst_reg = cpw.Sens_m(gpce_regression, joint_distribution)
sensFirst_ps = cpw.Sens_m(gpce_quadrature, joint_distribution)

sensT_reg = cpw.Sens_t(gpce_regression, joint_distribution)
sensT_ps = cpw.Sens_t(gpce_quadrature, joint_distribution)
