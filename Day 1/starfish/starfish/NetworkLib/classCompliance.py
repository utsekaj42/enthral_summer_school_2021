...
class LaplaceAdan(Laplace):
    """
    Laplace Compliance Model actually Hookean Model
    same model equations as Laplace, but it calculates the marterial parameter from 
    youngs modulus and wallThickness. WalThickness is calculated from eq. 17 in 
    "Boileau et al 'A benchmark study of numerical schemes for one-dimensional arterial blood flow modelling'"""
    def __init__(self, rho, As):
        super(LaplaceAdan, self).__init__(rho, As)
        self.wallThickness = None
        self.youngModulus  = None
