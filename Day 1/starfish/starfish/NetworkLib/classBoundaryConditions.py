...
class BoundaryConditionType1(BoundaryCondition):
    ...
    def initialize(self, bcDict):
        ...
        # # load file for Flow-From File
        if self.name == 'Pressure-FromFile':
            if self.loadedFile == False:
                self.loadFile()
    ...
    def findMeanPressure(self, P0=0, N=10000):
        #import time as timeModule
        self.initialize({})
        freq = self.freq
        period = 1/self.freq
        ...
        P_array = np.array([P0])
        for n, t in enumerate(time[:-1]):
            Q = self.calculateOneStep(n, dt)[0]
            P_array = np.append(P_array, Q)
        
                
        F = simps(P_array, time)
        
        P_mean = F/(time[-1] - time[0])

        self.TmeanFlow = 0
        self.initPhaseTimeSpan = 0

        return P_mean, self.initPhaseTimeSpan
...
class FlowFromFile(BoundaryConditionType1):
    ...
    def loadFile(self):
        ...
        try:
            reader = csv.DictReader(open(self.filePathName, 'r'), delimiter=';') #TODO: use a with
            #reader = csv.DictReader(open(self.filePathName, 'rb'), delimiter=';') #TODO: use a with
            # Why 'rb'??
        except Exception:
...
class PressureFromFile(BoundaryConditionType1):
    """
    Boundary profile - type 1
    ...
    def loadFile(self):
        ...
        try:
            ...
            for row in reader:
                dataTime.append(float(row['t']))
                dataPressure.append(float(row['P']))
            self.dataTime = np.asarray(dataTime)
            self.dataPressure = np.asarray(dataPressure)
            self.loadedFile = True
        except Exception: self.warning("Old except: pass clause in ccBC.PressureFromFile.loadFile")
    ...
    def function1(self, t, t0, pulsNum):
        ...
        tInter = np.interp(tReset, self.timeSim, self.timeData)
        # interpolate the Q value from data and dataTime
        P = np.interp(tInter, self.dataTime, self.dataPressure)
        # P = np.interp(t-pulseNumber*self.tmax,self.data.t,self.data.P)
        return P * self.duMatrix
...
class Windkessel2DAE(generalPQ_BC):
    ...
    def residualPQ(self, dP, dQ, P, Q, dt,nmem, n):
        return self.C*dP/dt + (P+dP-self.venousPressure[nmem])/self.Rc - (Q+dQ)
...
class Windkessel2(BoundaryConditionType2):
    ...
    def __call__(self, _domegaField_, duPrescribed, R, L,nmem,  n, dt, P, Q, A, Z1, Z2):
        return self.returnFunction(_domegaField_, R, dt, P, Q, nmem)
...
class Windkessel3(BoundaryConditionType2):
    ...
    def __call__(self, _domegaField_, duPrescribed, R, L, nmem, n, dt, P, Q, A, Z1, Z2):
        return self.returnFunction(_domegaField_, R, dt, P, Q, Z1, Z2, nmem)
...
class WindkesselMantero(BoundaryConditionType2):
    """
    Boundary profile - type 2

    5 Element Mantero Windkessel model
    ...
    def loadLvPressureFile(self):

        try:
            reader = csv.DictReader(open(self.filePathName, 'r'), delimiter=';') #TODO: use a with
        except Exception:
            self.exception("boundaryConditions.WindkesselMantero could not open file <<{}>> with boundary values, system exit".format(self.filePathName.split('/')[-1]))
        try:
            dataTime = []
            dataPressure = []
            for row in reader:
            ...
            self.T = dataTime[-1] - dataTime[0]
            t_new = np.linspace(dataTime[0], dataTime[-1], len(dataTime))
            P_new = np.interp(t_new, dataTime, dataPressure)
            self.LvTime = t_new - t_new[0]
            self.LvPressure = P_new
            
        except Exception: self.warning("Old except: pass clause in ccBC.WindkesselMantero.loadLvPressureFile")
    ...
    def funcPos1(self, domegaField_, R, dt, P, Q, Z1, Z2, n):
        """
        The two equations with the compliances are written in differential form (C*dP/dt=Q_c) and
        discretized with explicitt euler, P and Q at outlet of 1D vessel is written in terms of 
        domega values and right eigenvalue Matrix, and thus domega2 can be found from on differential eq
        and P_cm^n+1 from the other. The other variables; P_ca^n+1, Q_m^n+1, Q_d^n+1 are then updated with 
        the remaining equations from the 0D model
        """
        r11, r12, r21, r22 = R[0][0], R[0][1], R[1][0], R[1][1]

        
        if self.firstRun:
            # first guess of lumped variables (constant flow equal to initial values)
            
            if self.Rtotal != None:
                self.Rp = self.Rtotal*self.Rp_frac
                self.Rm = self.Rtotal*self.Rm_frac
                self.Rd = self.Rtotal*self.Rd_frac
                self.Ca = self.C*self.Ca_frac
                self.Cm = self.C*self.Cm_frac

                
            self.loadLvPressureFile()
            P_new = P
            Q_new = Q
            self.P_cm = (self.Rp + self.Rm)*Q_new
            

            self.P_ca = P_new - self.Rp*Q_new
            self.Qm = Q #(self.P_ca - self.P_cm)/self.Rm
            self.Qd = Q #(self.P_cm - self.venousPressure[n])/self.Rd
            self.firstRun = False
        ...
        t_eval = t - nCycle*self.T

        P_lv = np.interp(t_eval, self.LvTime, self.LvPressure)
        P_lv_next = np.interp(t_eval + dt, self.LvTime, self.LvPressure)
        
        
        _domega = dt*(Q - self.Qm)/(self.Ca*(r12 - self.Rp*r22)) - domegaField_*(r11 - self.Rp*r21)/(r12 - self.Rp*r22)
        self.P_cm = dt*(self.Qm - self.Qd)/self.Cm + self.P_cm - P_lv + P_lv_next
        
        P_new = P + r11*domegaField_ + r12*_domega
        Q_new = Q + r21*domegaField_ + r22*_domega
        self.P_ca = P_new - self.Rp*Q_new
        self.Qm = (self.P_ca - self.P_cm)/self.Rm
        self.Qd = (self.P_cm - self.venousPressure[n])/self.Rd
