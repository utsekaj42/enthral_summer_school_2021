...
def readConfigFile(options):
    ...
    for option in options:    
        if option == 'WorkingDirectory':
            #try:
            workingDirectory = config.get('Directory Paths', option, fallback='')
            if os.path.isdir(workingDirectory) == False:
                print(Warning("\n ERROR WorkingDirectory {} does not exist \n".format(workingDirectory)))
                
                knownWorkingDirectories = config.get('Directory Paths', 'knownWorkingDirectories', fallback='').split(',')
...
def workingDirectorySettings(searchKnowWorkingDirectories = True):
    ...
    '''
    knownWorkingDirectories = readConfigFile('knownWorkingDirectories')
    choices = ['add working directory',]
    if len(knownWorkingDirectories)>0:
        choices.append('switch to another known working directory')
    #prettyPrintList(' Working directory settings menu',['add working directory','switch to another known working directory'])
    prettyPrintList(' Working directory settings menu', choices)
    ...
    userInput = userInputEvaluationInt(len(choices))
    # TODO fix options
    if userInput == 0:
