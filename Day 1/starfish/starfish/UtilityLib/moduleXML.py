...
from starfish.CoronaryLib.classStenosesInputManager import StenosesInputManager
...
def writeNetworkToXML(vascularNetwork, dataNumber = "xxx", networkXmlFile = None):
    ...
    # TODO fix relative file paths upon write?
    ...
    for xmlElementName in nxmlW.xmlElements:
        ...
        elif xmlElementName == 'stenosesInputManager':
            if vascularNetwork.stenosesInputManager is not None:
                vascularNetwork.stenosesInputManager.writeDataToXmlNode(xmlFileElement)
                xmlFileElement.set('class', 'StenosesInputManager')
        ...
        for variableType in variableTypes:
            ...
            if variableType in ['float','int']:
                ...
                try:
                    ...
                    print("Warning: loadVariablesConversion() list: variableValueString is not a valid expression for eval()")
                    convertError.append('list')
                except NameError:
                    print("Warning: loadVariablesConversion() list: variableValueString is not defined")
                    convertError.append('list')
    ...
    for xmlElementName in nxml.xmlElements:
        for xmlElement in root.findall(''.join([".//",xmlElementName])):
            ...
            if xmlElementName == 'boundaryConditions':
                        ...
                        for bcElements in boundaryConditionElement.findall(''.join(['.//',boundaryType])):
                            ...
                            for variable in nxml.xmlElementsReference[xmlElementName][boundaryType]:
                                ...
                                if variable == 'filePathName':
                                    ...
                                    variableValueStr = os.path.join(networkDirectory, variableValueStr)
                ...
                xmlElementChildren = xmlElement.getchildren()
                if len(xmlElementChildren) != 0:
                    stenosesInputManager = StenosesInputManager()
                    vascularNetwork.stenosesInputManager = stenosesInputManager
                    vascularNetwork.stenosesInputManager.loadDataFromXmlNode(xmlElement)
