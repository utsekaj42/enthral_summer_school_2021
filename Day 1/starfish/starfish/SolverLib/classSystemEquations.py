...
class System(cSBO.StarfishBaseObject):
    ...
    def __init__(self, vessel, simplifyEigenvalues, riemannInvariantUnitBase, currentMemoryIndex, currentTimeStep, dt, field=None, twoStep=False):
        """
...
#             
        ...
        # set up B matrix
        b2 = (-2.0 / self.rho * pi * v * (self.gamma+2) * self.my + A * self.netGravity[self.currentMemoryIndex[0]]) #Correction with net gravity force
    ...
    def updateLARLSys0InvariantFlow(self,P,Q,A,position):
        ...
        domega = np.dot( L[1+position], du) + self.dt * L[1+position][1] * A[position] * self.netGravity[self.currentMemoryIndex[0]]
    ...
    def updateLARLSys1InvariantFlow(self,P,Q,A,position,sqrt=np.sqrt):
        ...
        domega = np.dot( L[1+position], du) + self.dt * L[1+position][1] * Aid * self.netGravity[self.currentMemoryIndex[0]]
