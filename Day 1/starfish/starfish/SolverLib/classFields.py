...
class Field():
    ...
    def __init__(self, vessel, currentMemoryIndex, dt, rigidArea, solvingSchemeField = 'MacCormack_Flux'):
        ...
        self.u = np.zeros((2, len(self.P_pre)))
        self.up = np.zeros_like(self.u)
        # flux_a and flux_b are intermediate memory (no semantic meaning of a and b. Used as (flux_a - flux_b)/dt now.
        self.flux_a = np.zeros((self.u.shape[0], self.u.shape[1]-1))
        self.flux_b = np.zeros((self.u.shape[0], self.u.shape[1]-1))
    ...
    def __call__(self):
        """ Need to def __call__ for numerical object... could have a "base numerical object" """
        return self._callfct()


    def F(self,flux_arr, u, A, C, Aconst, Cconst):
    ...
    def MacCormackFlux(self):
        ...
        C1 = C[1:]
        C2 = C[:-1]
        up[:,:-1] = u[:, :-1] - dt*(self.F(self.flux_a, u[:, 1:], A1, C1, Aconst1, Cconst1) -
                                    self.F(self.flux_b, u[:, :-1], A2, C2, Aconst1, Cconst1))/dx
        ...
        Aconst2 = A_p[1:]
        Cconst2 = C_p[1:]
        u[:,1:] = .5*(u[:, 1:] + up[:, 1:] -  dt/dx*(self.F(self.flux_a, up[:, 1:], A_p1, C_p1, Aconst2, Cconst2) -
                                                     self.F(self.flux_b, up[:, :-1], A_p2, C_p2, Aconst2, Cconst2)))
        ...
        self.P[n + 1][1:-1] = Pnewinterior[1:-1]
        self.Q[n + 1][1:-1] = Qnewinterior[1:-1]
        self.A[n + 1][1:-1] = Anewinterior[1:-1]

        #TODO: Please explain this if statement in a comment.
        if (self.P[n+1] < -500*133.32).any():
            #import matplotlib.pylab as plt
            
            c = self.vessel.waveSpeed(A, C)
            U = Q/A
            U_plus_c_max = max(c + U)
            
            U_max = max(U)
            c_max = max(c)
            #plt.plot(P/133.32)
            CFL_max = dt*U_plus_c_max/dx
            #plt.show()
            raise ValueError("ERROR: {} calculated negative pressure in corrector step at time {} (n {}, dt {}, CFL {}, U {}, c {}), exit system".format(self.name,n*dt,n,dt, CFL_max, U_max, c_max))
