...
class NetworkReduction(cSBO.StarfishBaseObject):
    ...
    def __init__(self, vascularNetwork, quiet=False):
        ...
        self.stenosesInputManager = vascularNetwork.stenosesInputManager
        
        self.geometryPath = vascularNetwork.geometryPath
        ...
        print(self.centralVenousPressure)
        #print self.vessels
    ...
    def updateWK3params(self, wk3BC, Z, Rc, Rtotal, C):
        wk3BC.Z = Z
        wk3BC.Rc = Rc
        wk3BC.Rtotal = Rtotal
        wk3BC.C = C
    ...
    def reduceNetworkFromListGen(self, truncateList):
        ...
        vesselsToRemove, bcsToRemove = self.findVesselsAndBCsToRemoveV2(truncateList)
        
        for n, vesselId in enumerate(truncateList):
            bcVesselToCopy = bcsToRemove[0]
            print(self.boundaryConditions[bcVesselToCopy][0].name, 
                    'Mantero' in self.boundaryConditions[bcVesselToCopy][0].name)
            if self.useLumpedValues:
                
                Rtot = self.lumpedValues[vesselId]["R"][-1]
                Z_0 = self.lumpedValues[vesselId]["R1"][-1]
                if 'Mantero' in self.boundaryConditions[bcVesselToCopy][0].name:
            ...
            if vesselId in self.boundaryVessels:
                self.boundaryConditions[vesselId].append(copy.deepcopy(self.boundaryConditions[bcVesselToCopy][0]))
                self.updateWK3params(self.boundaryConditions[vesselId][1], Z_0, R, Rtot, Cnew)
                ...
                self.boundaryConditions[vesselId] = [copy.deepcopy(self.boundaryConditions[bcVesselToCopy][0])]
                
                self.updateWK3params(self.boundaryConditions[vesselId][0], Z_0, R, Rtot, Cnew)
                print('yolo')
    ...
    def reduceTerminalBifurcationOpt(self, vesselId, N=501):
        ...
        if self.useAverageValues:
            Z_0 = 10**6*133.32*self.optParamsDict[vesselId][node][WkMethod][params]['R1_0']
            R = 10**6*133.32*self.optParamsDict[vesselId][node][WkMethod][params]['R2_0']
            Cnew =  10**-6*self.optParamsDict[vesselId][node][WkMethod][params]['C_0']/133.32
        else:
            Z_0 = 10**6*133.32*self.optParamsDict[vesselId][node][WkMethod][params]['R1']
            R = 10**6*133.32*self.optParamsDict[vesselId][node][WkMethod][params]['R2']
            Cnew =  10**-6*self.optParamsDict[vesselId][node][WkMethod][params]['C']/133.32
...
#         self.boundaryVessels.append(vesselId)
    ...
    def findVesselsAndBCsToRemoveV2(self, truncateList):
        vesselsToRemove = []
        bcsToRemove = []
        ...
        for vesselIdTruncate in truncateList:
            ...
            listOfDaughters = self.vessels[vesselIdTruncate].daughterList
            
            for vesselId_d in listOfDaughters:
                if vesselId_d not in toVisit:
                    toVisit.append(vesselId_d)
            while len(toVisit) > 0:
                
                for vesselId in toVisit:
                    listOfDaughters = self.vessels[vesselId].daughterList
                    
                    if vesselId not in vesselsToRemove:
                        vesselsToRemove.append(vesselId)
                    
                    if len(listOfDaughters) == 0 and vesselId not in bcsToRemove:
                        bcsToRemove.append(vesselId)
